    ...
    <FormField
      ...
      render={({ field: { onChange, ...otherFields } }) => (
        <FormItem className="group relative h-full flex-1 grow space-y-0 pt-2 transition-[width,flex] duration-300">
          <FormLabel className="absolute top-0 z-10 max-w-full truncate bg-background px-2 text-xs text-foreground">
            {label}
          </FormLabel>
          <FormControl>
            <div className="relative h-full">
              <Input
                ...
                {...otherFields}
              />
              <div className="pointer-events-none absolute right-10 top-0 hidden h-full items-center pl-3 group-hover:flex">
